...
#define JS_PS_END JS_PSGS(0, 0, 0, 0)
    ...
    namespace {
        ...
        JSValueRef _getPropertyCallback(JSContextRef context, JsValueRef object, JSStringRef propertyName, JSValueRef* exception)
        {
        ...
        bool _setPropertyCallback(JSContextRef context, JsValueRef object, JSStringRef propertyName, JSValueRef value, JSValueRef* exception)
        {
            SE_UNUSED_PARAM(context);
            SE_UNUSED_PARAM(object);
        ...
        bool _hasPropertyCallback(JSContextRef context, JsValueRef object, JSStringRef propertyName)
        {
            SE_UNUSED_PARAM(context);
            SE_UNUSED_PARAM(object);
    ...
    bool Class::install()
    {
        ...
        JsValueRef jsCtor = JSObjectMakeConstructor(__cx, _jsCls, _ctor);

        for (const auto& staticfunc : _staticFuncs)
        {
            JSStringRef name = JSStringCreateWithUTF8CString(staticfunc.name);
            JsValueRef func = JSObjectMakeFunctionWithCallback(__cx, nullptr, staticfunc.callAsFunction);
    ...
    JsValueRef Class::_createJSObject(const std::string &clsName, Class** outCls)
    {
        ...
        JsValueRef obj = JSObjectMake(__cx, thiz->_jsCls, nullptr);
        *outCls = thiz;
        return obj;
    }
