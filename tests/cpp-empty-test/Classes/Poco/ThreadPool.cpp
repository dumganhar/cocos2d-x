...
{
public:
	PooledThread(const std::function<void()>& notify, const std::string& name, int stackSize = POCO_THREAD_STACK_SIZE);
	~PooledThread();
...
PooledThread::PooledThread(const std::function<void()>& notify, const std::string& name, int stackSize):
	_idle(true), 
	_idleTime(0), 
	_pTarget(nullptr),
	_name(name), 
...
{
    auto pooledThread = getThread();
    if (pooledThread)
    {
        pooledThread->start(Thread::PRIO_NORMAL, target);
    }
    else
    {
        FastMutex::ScopedLock lock(_pendingMutex);
        _pendingTasks.push(target);
    }
}
...
void ThreadPool::start(const Runnable& target, const std::string& name)
{
    ...
    if (pooledThread)
    {
        pooledThread->start(Thread::PRIO_NORMAL, target, name);
    }
    else
    {
        FastMutex::ScopedLock lock(_pendingMutex);
...
void ThreadPool::startWithPriority(Thread::Priority priority, const Runnable& target)
{
    ...
    if (pooledThread)
    {
        pooledThread->start(priority, target);
    }
    else
    {
        FastMutex::ScopedLock lock(_pendingMutex);
...
void ThreadPool::startWithPriority(Thread::Priority priority, const Runnable& target, const std::string& name)
{
    ...
    if (pooledThread)
    {
        pooledThread->start(priority, target, name);
    }
    else
    {
        FastMutex::ScopedLock lock(_pendingMutex);
...
PooledThread* ThreadPool::getThread()
{
	...
	if (!pThread)
	{
		...
		else //cjh: DONT throw, put it to queue.  throw NoThreadAvailableException();
        {
...
{ // This is in sub thread
    std::function<void()> pendingTask = nullptr;
    _pendingMutex.lock();
    if (!_pendingTasks.empty())
    {
        pendingTask = _pendingTasks.front();
        _pendingTasks.pop();
    }
...
PooledThread* ThreadPool::createThread()
{
	...
	name << _name << "[#" << ++_serial << "]";
	return new PooledThread(std::bind(&ThreadPool::onTaskFinished, this), name.str(), _stackSize);
}
